...
- **Verification:**
	- `curl -s -u  http://127.0.0.1:3000/api/datasources/uid/victoriametrics`
	- `curl -s -u  -H 'Content-Type: application/json' -X POST http://127.0.0.1:3000/api/ds/query -d '{... "expr":"node_uname_info" ...}'` → повертаються series з `job="node-exporter"`.
